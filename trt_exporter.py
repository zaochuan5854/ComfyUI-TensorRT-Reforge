...
if __name__ == "__main__":
    # onnx, onnx_data, engine = load_onnx_and_trt_engine("/opt/comfyui/models/tensorrt/tensorrt_model.onnx_and_engine.zip")
    import sys
    import os
    import traceback
    from typing import Any
    from comfy.sd import load_checkpoint_guess_config # pyright: ignore[reportUnknownVariableType]
    
    def main():
        ckpt_path = R"/opt/comfyui/models/checkpoints/ponyDiffusionV6XL_v6StartWithThisOne.safetensors"
        
        if not os.path.exists(ckpt_path):
            print(f"エラー: モデルが見つかりません -> {ckpt_path}")
            return

        print(f"モデルをロード中: {ckpt_path}")
        try:
        ...
        kwargs: TRTSpec = {
            "model_patcher": model_patcher, # pyright: ignore[reportAssignmentType]
            "opt_batch_size": 1,
            "opt_width": 512,
            "opt_height": 512,
            "opt_context_mult": 1,
            "filename_prefix": "tensorrt/",
            "min_batch_size": 0,
            "max_batch_size": 0,
            "min_width": 0,
            "max_width": 0,
        ...
        print("TRTExporter.execute() を呼び出します...")
        try:
            with torch.inference_mode():
                result = TRTExporter.execute(**kwargs)
